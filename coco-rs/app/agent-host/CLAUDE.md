...
| `session::task_runtime` | Running-task lifecycle, output and notification ownership; shell prompt stalls and phase-aware local-agent liveness have separate watchdogs. Liveness limits come from `RuntimeConfig.agent_liveness` — inactivity-only by default; the absolute wall-clock kill is opt-in, since an agent still emitting output is working. |
