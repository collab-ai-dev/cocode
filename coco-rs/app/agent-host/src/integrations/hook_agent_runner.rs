...
async fn run_agent(
    ...
    let hook_session_id = configure_hook_agent(&mut config, &request);

    let cancel = CancellationToken::new();
    let mut engine = session
        .build_engine_from_config_with_registries(config, hook_session_id, cancel, tools, None)
        .await
...
/// Configure the cloned engine config for a hook-agent run and return the
/// distinct hook-agent session id (kept off the mutable config surface).
fn configure_hook_agent(
    config: &mut QueryEngineConfig,
    request: &HookAgentRunRequest,
) -> coco_types::SessionId {
    ...
    match coco_types::SessionId::try_new(session_id) {
        Ok(id) => id,
