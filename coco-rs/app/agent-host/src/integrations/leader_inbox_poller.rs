...
pub fn spawn(session: SessionHandle) {
    ...
    // Tracked as a session-owned task so close aborts and joins it. The poll
    // loop has no cooperative stop, so without this it would leak past close
    // (CS-3 session task supervisor).
    session.spawn_session_task(async move {
        ...
        loop {
            poll_once(&poll_session, &mut dispatched).await;
