...
//!
//! ## Why this lives in app/agent-host (and not in coco-tasks)
//!
//! `coco-tasks` is layered below `coco-query` (where `CommandQueue`
//! lives). Pushing the sink impl up into `app/agent-host` keeps the
//! dependency direction acyclic. The producer side
//! (`TaskManager::*` / `TaskRuntime::*`) talks to the trait;
//! `app/agent-host` is the only place that knows about both the trait and
