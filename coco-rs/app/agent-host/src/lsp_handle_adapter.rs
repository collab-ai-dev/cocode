...
//!
//! Lives in `app/agent-host` because it sits at the seam where both crates
...
impl LspHandle for LspManagerAdapter {
    ...
    async fn send_request(
        ...
        debug!(target: "coco_agent_host::lsp", method, path = %file_path.display(), "dispatching LSP request");
    ...
    async fn notify_save(&self, file_path: &Path) {
        ...
        let client = match self.manager.get_client(file_path).await {
            ...
            Err(err) => {
                ...
                debug!(target: "coco_agent_host::lsp", path = %file_path.display(), %err, "notify_save skipped (no server)");
                return;
            }
        };

        if let Err(err) = client.notify_save(file_path).await {
            warn!(target: "coco_agent_host::lsp", path = %file_path.display(), %err, "LSP didSave failed");
        }
    }
