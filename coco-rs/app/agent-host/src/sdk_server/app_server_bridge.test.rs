...
use crate::sdk_server::idle_session_supervisor::spawn_idle_session_sweep;
...
async fn app_server_local_bridge_archives_registered_surfaces() {
    ...
    assert_eq!(
        lifecycle.kind,
        coco_types::SurfaceLifecycleEffectKind::SessionEnded {
            session_id: started.session_id.clone(),
        }
    );
    assert!(
...
#[tokio::test]
async fn idle_session_supervisor_waits_for_detach_then_archives_at_deadline() {
    let app_server = Arc::new(AppServer::<LocalAppSessionHandle>::new(1, 8));
    let state = Arc::new(SdkServerState::default());
    let session_id = SessionId::try_new("sess-idle-event-driven").expect("valid session id");
    register_local_app_server_session(
        &app_server,
        LocalAppSessionHandle::snapshot(session_id.clone()),
    )
    .await
    .expect("register idle session");

    let adapter = LocalClientAdapter::with_channel_capacity(Arc::clone(&app_server), 8);
    let connection = adapter.connect();
    let surface = connection
        .attach_surface(session_id, AttachSurfaceOptions::default())
        .expect("attach passive surface");
    let supervisor = spawn_idle_session_sweep(
        Arc::clone(&app_server),
        state,
    ...
    assert_eq!(app_server.list_live_sessions().len(), 1);

    connection.detach_surface(&surface.surface_id);
    tokio::time::timeout(Duration::from_millis(500), async {
        while !app_server.list_live_sessions().is_empty() {
            tokio::time::sleep(Duration::from_millis(5)).await;
        }
    })
    .await
    .expect("detached session should archive at its activity deadline");
    supervisor.abort();
}
...
async fn local_lifecycle_resume_replaces_detached_live_session_before_attaching_new_surface() {
    ...
    assert_eq!(
        lifecycle.kind,
        coco_types::SurfaceLifecycleEffectKind::SessionReplaced {
            old_session_id,
            new_session_id,
        }
    );
...
async fn local_lifecycle_resume_replaces_interactive_live_session() {
    ...
    assert_eq!(
        lifecycle.kind,
        coco_types::SurfaceLifecycleEffectKind::SessionStarted {
...
/// therefore continues numbering instead of restarting at 1.
#[tokio::test]
async fn session_seq_continues_across_forwarder_respawn() {
    ...
    send_session_event(&tx1, session_id.clone(), running())
        .await
        .expect("send via forwarder 1");
    ...
    send_session_event(&tx2, session_id.clone(), running())
        .await
        .expect("send via forwarder 2");
...
async fn sdk_bridge_enqueues_protocol_notifications_to_hub_connector() {
    ...
    let adapter = JsonRpcAdapter::with_channel_capacity(Arc::clone(&server), 8);
    let connection = adapter.connect();
    let surface_id = SurfaceId::from("surface-sdk-stdio");
    server
    ...
    assert_eq!(notification.params["session_id"], session_id.as_str());
    assert_eq!(notification.params["surface_id"], surface_id.as_str());
    assert_eq!(notification.params["session_seq"], 1);

    let replay_adapter = LocalClientAdapter::with_channel_capacity(Arc::clone(&server), 8);
    let replay_connection = replay_adapter.connect();
    let replay = replay_connection
        .subscribe_surface(
            session_id.clone(),
            Some(0),
            AttachSurfaceOptions {
                role: coco_app_server::SurfaceRole::Passive,
                ..AttachSurfaceOptions::default()
            },
        )
        .expect("subscribe for replay");
    let coco_app_server::LocalClientSubscribeOutcome::Attached(replay) = replay else {
        panic!("expected retained SDK event");
    };
    assert_eq!(replay.replayed.len(), 1);
    assert_eq!(replay.replayed[0].session_seq, Some(1));
