...
async fn commands_filters_sensitive_commands() {
    ...
    // Sensitive: must NOT appear on the initialize wire even though
    ...
    assert!(
        ...
        "sensitive commands must not be advertised to initialize clients"
    );
}
...
async fn account_api_key_maps_to_first_party() {
    ...
    assert_eq!(
        ...
        Some(SessionAccountProvider::FirstParty)
    );
...
async fn account_oauth_does_not_leak_access_token() {
    ...
    let account = boot.account().await;
    // Debug the host metadata and search for secret material — regression
    // guard against accidentally adding a field that forwards the token.
...
async fn account_third_party_providers_return_empty_account() {
    // Non-FirstParty providers return empty first-party account metadata.
    use coco_inference::auth::AuthMethod;
