...
impl ControlRequestHandler for AppServerBridgeControlHandler {
    async fn handle(&self, request: ControlRequest) -> Result<serde_json::Value, ControlError> {
        ...
        match request {
            ControlRequest::Initialize { system_prompt: _ } => {
                // system_prompt is per-session execution policy and now belongs
                // on session/start, not initialize; the REPL bridge Initialize
                // control no longer forwards it (the initialize handler ignored
                // it previously).
