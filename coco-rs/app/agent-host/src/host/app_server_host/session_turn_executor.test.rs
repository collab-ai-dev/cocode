...
//! rebuild the session-runtime bootstrap. End-to-end behavior is
//! exercised via the CLI integration path; `ScriptedRunner` in `dispatcher.test.rs` is
//! the unit-level stand-in for the `TurnRunner` trait contract.
//!
//! What we keep here is the compile-time Send+Sync assertion: the
//! `AppServerHostState` holds `Arc<dyn TurnRunner>` across await points,
...
#[test]
fn goal_status_format_matches_noninteractive_contract() {
