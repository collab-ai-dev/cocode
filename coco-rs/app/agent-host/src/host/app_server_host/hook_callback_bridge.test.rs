use coco_types::HookCallbackOutput;
...
fn pre_tool_use_deny_round_trips_through_hook_specific_output() {
    ...
    // Canonical hook/callback reply shape: `{output}`, where
    ...
    assert_eq!(specific["permissionDecisionReason"], "client denied");
...
/// Top-level `continue: false` propagates through HookCallbackOutput as
