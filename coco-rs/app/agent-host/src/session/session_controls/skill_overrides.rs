...
pub struct SkillOverridesUpdate {
    pub result: coco_event_types::SkillOverridesSaveResult,
...
pub async fn write_skill_overrides(
    ...
    match write_result {
        Ok(()) => {
            ...
            SkillOverridesUpdate {
                result: coco_event_types::SkillOverridesSaveResult::Ok,
                commands: Some(commands),
            }
        }
        Err(error) => SkillOverridesUpdate {
            result: coco_event_types::SkillOverridesSaveResult::Err {
...
) -> coco_event_types::SkillOverridesSaveErrorKind {
    use coco_config::SettingsWriteError as Error;
    use coco_event_types::SkillOverridesSaveErrorKind as Kind;
    match error {
        Error::Io { .. } => Kind::Io,
