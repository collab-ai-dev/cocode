...
pub(in crate::session::session_runtime) struct SessionIntegrationResources {
    ...
    pub(in crate::session::session_runtime) mcp_manager:
        Arc<RwLock<Option<Arc<tokio::sync::Mutex<coco_mcp::McpConnectionManager>>>>>,
    /// Monotonic "the MCP server set changed" signal.
    pub(in crate::session::session_runtime) mcp_reconnect_key: Arc<AtomicU64>,
    /// Late-bound LSP handle installed on every per-turn engine.
    pub(in crate::session::session_runtime) lsp_handle:
        Arc<RwLock<Option<coco_tool_runtime::LspHandleRef>>>,
    pub(in crate::session::session_runtime) reload_supervisor:
    ...
    pub(in crate::session::session_runtime) mcp_registration_reports:
...
impl SessionIntegrationResources {
    ...
    pub(in crate::session::session_runtime) fn mcp_handle(
        &self,
    ) -> &Arc<RwLock<Option<coco_tool_runtime::McpHandleRef>>> {
        &self.mcp_handle
    }

    pub(in crate::session::session_runtime) fn mcp_manager(
        &self,
    ) -> &Arc<RwLock<Option<Arc<tokio::sync::Mutex<coco_mcp::McpConnectionManager>>>>> {
        &self.mcp_manager
    }

    pub(in crate::session::session_runtime) fn mcp_reconnect_key(&self) -> &Arc<AtomicU64> {
        &self.mcp_reconnect_key
    }

    pub(in crate::session::session_runtime) fn lsp_handle(
        &self,
    ) -> &Arc<RwLock<Option<coco_tool_runtime::LspHandleRef>>> {
        &self.lsp_handle
    }

    pub(in crate::session::session_runtime) fn reload_supervisor(
        &self,
    ) -> &Arc<Mutex<Option<tokio::task::JoinHandle<()>>>> {
        &self.reload_supervisor
    }

    pub(in crate::session::session_runtime) fn mcp_registration_reports(
        &self,
    ) -> &Arc<RwLock<HashMap<String, crate::session_runtime::McpRegistrationStatus>>> {
...
#[derive(Clone)]
pub(in crate::session::session_runtime) struct SessionHandleResources {
    /// Eager no-op-capable agent handle installed on every engine before the
    /// concrete coordinator-backed handle is late-bound.
    pub(in crate::session::session_runtime) swarm_agent_handle: coco_tool_runtime::AgentHandleRef,
    /// Real `AgentHandle` for `AgentTool` calls and forked subagents.
    pub(in crate::session::session_runtime) agent_handle: Arc<RwLock<Option<AgentHandleRef>>>,
...
#[derive(Clone)]
pub(in crate::session::session_runtime) struct SessionPermissionResources {
    /// Teammate-scoped live permission-rule overlay injected into each main
    /// session engine config.
    pub(in crate::session::session_runtime) live_permission_rules:
        Arc<RwLock<Vec<coco_types::PermissionRule>>>,
}

impl SessionPermissionResources {
    pub(in crate::session::session_runtime) fn new() -> Self {
        Self {
...
#[derive(Clone)]
pub(in crate::session::session_runtime) struct SessionAgentCatalogResources {
    /// Where the agent loader looks for markdown agents. Plugin reload
    /// refreshes this from the latest project-services snapshot.
    pub(in crate::session::session_runtime) agent_search_paths:
        Arc<RwLock<coco_subagent::definition_store::AgentSearchPaths>>,
    /// Built-in agent toggles applied to every reload.
    pub(in crate::session::session_runtime) builtin_agent_catalog:
        coco_subagent::BuiltinAgentCatalog,
    /// Active per-session agent catalog snapshot.
    pub(in crate::session::session_runtime) agent_catalog:
    ...
    pub(in crate::session::session_runtime) client_supplied_agents:
...
#[derive(Clone)]
pub(in crate::session::session_runtime) struct SessionMemoryResources {
    /// Auto-memory runtime — extraction / dream / session memory / recall
    /// ranker. `None` when `Feature::AutoMemory` is off.
    pub(in crate::session::session_runtime) memory_runtime: Option<Arc<coco_memory::MemoryRuntime>>,
    /// Skill-learning review runtime. `Some` when `Feature::SkillLearning`
    /// is enabled.
    pub(in crate::session::session_runtime) skill_review_runtime:
...
#[derive(Clone)]
pub(in crate::session::session_runtime) struct SessionSandboxResources {
    /// Session-scoped sandbox state shared by engines, AppServer controls, and fork
    /// dispatch.
    pub(in crate::session::session_runtime) sandbox_state: Option<Arc<coco_sandbox::SandboxState>>,
}
...
#[derive(Clone)]
pub(in crate::session::session_runtime) struct SessionHistoryResources {
    /// Multi-turn agent transcript shared across per-turn engines and runtime
    /// control paths.
    pub(in crate::session::session_runtime) history: Arc<Mutex<MessageHistory>>,
}

impl SessionHistoryResources {
    pub(in crate::session::session_runtime) fn new(history: Arc<Mutex<MessageHistory>>) -> Self {
        Self { history }
    }

    pub(in crate::session::session_runtime) fn history(&self) -> &Arc<Mutex<MessageHistory>> {
        &self.history
    }
}
