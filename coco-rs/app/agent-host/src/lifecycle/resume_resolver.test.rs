...
fn cli_with(args: &[&str]) -> AgentHostOptions {
    let mut options = AgentHostOptions::default();
    let mut args = args.iter().copied();
    while let Some(arg) = args.next() {
        match arg {
            "--resume" => options.resume = args.next().map(str::to_string),
            "--continue" => options.continue_session = true,
            "--fork-session" => options.fork_session = true,
            "--session-id" => options.session_id = args.next().map(str::to_string),
