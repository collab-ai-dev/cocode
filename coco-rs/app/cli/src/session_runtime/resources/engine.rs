...
pub(in crate::session_runtime) struct SessionWorkspaceResources {
    ...
    pub(in crate::session_runtime) original_cwd: PathBuf,
    /// Git worktree root for project-scoped services, or
    /// [`Self::original_cwd`] when the session is outside git.
    pub(in crate::session_runtime) project_root: PathBuf,
    ...
    pub(in crate::session_runtime) current_cwd: Arc<RwLock<PathBuf>>,
}
...
impl SessionWorkspaceResources {
    ...
    pub(in crate::session_runtime) fn original_cwd(&self) -> &PathBuf {
        &self.original_cwd
    }

    pub(in crate::session_runtime) fn project_root(&self) -> &PathBuf {
        &self.project_root
    }

    pub(in crate::session_runtime) fn current_cwd(&self) -> &Arc<RwLock<PathBuf>> {
        &self.current_cwd
    }
}

#[derive(Clone)]
pub(in crate::session_runtime) struct SessionEngineConfigResources {
    /// Engine config; mutated by [`crate::session_runtime::SessionRuntime::update_engine_config`] and
    /// read by every per-turn build.
    pub(in crate::session_runtime) engine_config: Arc<RwLock<QueryEngineConfig>>,
    ...
    pub(in crate::session_runtime) orchestration_engine_config:
    ...
    pub(in crate::session_runtime) role_overrides: Arc<RwLock<HashMap<ModelRole, RoleOverride>>>,
}
...
impl SessionEngineConfigResources {
    ...
    pub(in crate::session_runtime) fn engine_config(&self) -> &Arc<RwLock<QueryEngineConfig>> {
        &self.engine_config
    }

    pub(in crate::session_runtime) fn orchestration_engine_config(
        &self,
    ) -> &Arc<std::sync::RwLock<QueryEngineConfig>> {
        &self.orchestration_engine_config
    }

    pub(in crate::session_runtime) fn role_overrides(
        &self,
...
#[derive(Clone)]
pub(in crate::session_runtime) struct SessionEngineStateResources {
    pub(in crate::session_runtime) file_read_state: Arc<RwLock<FileReadState>>,
    pub(in crate::session_runtime) file_history: Option<Arc<RwLock<FileHistoryState>>>,
    pub(in crate::session_runtime) app_state: Arc<RwLock<ToolAppState>>,
    ...
    pub(in crate::session_runtime) session_env_vars: Option<coco_shell::SessionEnvVars>,
    ...
    pub(in crate::session_runtime) loop_sentinel_state:
...
impl SessionEngineStateResources {
    ...
    pub(in crate::session_runtime) fn file_read_state(&self) -> &Arc<RwLock<FileReadState>> {
        &self.file_read_state
    }

    pub(in crate::session_runtime) fn file_history(
        &self,
    ) -> Option<&Arc<RwLock<FileHistoryState>>> {
        self.file_history.as_ref()
    }

    pub(in crate::session_runtime) fn app_state(&self) -> &Arc<RwLock<ToolAppState>> {
        &self.app_state
    }

    pub(in crate::session_runtime) fn session_env_vars(
        &self,
    ) -> Option<&coco_shell::SessionEnvVars> {
        self.session_env_vars.as_ref()
    }

    pub(in crate::session_runtime) fn loop_sentinel_state(
        &self,
    ) -> &Arc<Mutex<coco_skills::bundled::loop_skill::LoopSentinelState>> {
        &self.loop_sentinel_state
    }

    pub(in crate::session_runtime) fn pending_message_store(
        &self,
    ) -> &coco_tool_runtime::PendingMessageStoreRef {
        &self.pending_message_store
    }

    pub(in crate::session_runtime) fn auto_mode_state(
        &self,
    ) -> &Arc<coco_permissions::AutoModeState> {
        &self.auto_mode_state
    }

    pub(in crate::session_runtime) fn denial_tracker(
        &self,
    ) -> &Arc<tokio::sync::Mutex<coco_permissions::DenialTracker>> {
        &self.denial_tracker
    }

    pub(in crate::session_runtime) fn transcript_dedup(
        &self,
    ) -> &Arc<tokio::sync::Mutex<std::collections::HashSet<uuid::Uuid>>> {
        &self.transcript_dedup
    }

    pub(in crate::session_runtime) fn clear_rewind_messages(
        &self,
    ) -> &Arc<tokio::sync::Mutex<Option<Vec<Arc<Message>>>>> {
        &self.clear_rewind_messages
    }

    pub(in crate::session_runtime) fn terminal_goal_metadata_written(&self) -> &Arc<AtomicBool> {
