        ...
        match stop_result {
            ...
            Ok(agg) if agg.is_blocked() => {
                if let Some(err) = &agg.blocking_error {
                    ...
                    // **Finding R3** — resets `maxOutputTokensRecoveryCount: 0`
                    // on the stop-hook-blocking continue: the branch is a fresh
                    // attempt from the user-prompt perspective, so the recovery
                    // counter (which belongs to the previous attempt) resets.
                    turn_state.max_tokens_recovery_count = 0;
                    StopHookDecision::BlockedContinueLoop
                } else {
                    // Defensive: `is_blocked()` true but `blocking_error` None.
                    // Shouldn't happen given the aggregator's invariants.
                    StopHookDecision::Continue
                }
            }
            Ok(_agg) => StopHookDecision::Continue,
            Err(e) => {
