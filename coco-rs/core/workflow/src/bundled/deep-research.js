...
// WebSearch/WebFetch instead of git/grep.
...
// ─── Schemas ───
// Every agent() call carries one. The enums are not documentation: the rank
// tables below index them directly, so a schema-validated enum is what makes
// the sort comparators total. An unvalidated string would yield undefined,
// and (undefined - n) is NaN, which makes Array.prototype.sort inconsistent.
const SCOPE_SCHEMA = {
    ...
    return parallel(
      novel.map(source => () => {
        ...
        return agent(FETCH_PROMPT(source, searchResult.angle), {
          ...
          // User-skip / policy-refusal → null; drop it (filtered by
          // searchResults.flat().filter(Boolean)) rather than throwing into
...
const voted = (await parallel(
  rankedClaims.map(claim => () =>
    parallel(
      ...
      // A vote can be null (user-skip, policy refusal, or agent error) — treat
      // as no vote cast. Three outcomes (infra failure must not read as
      // "refuted"):
