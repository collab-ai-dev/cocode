...
//! onto the operation-scoped [`VoiceProgress`] stream.
...
/// models, and forward progress to the operation-scoped sink.
/// Returns the installed path. `cancel` aborts mid-transfer (leaving no partial
/// file behind).
pub async fn download_model(
    config: &LocalWhisperConfig,
    events: Option<mpsc::Sender<VoiceProgress>>,
    ...
    // Bridge lossy downloader progress onto this transcription operation.
...
fn map_progress(model: &str, progress: DownloadProgress) -> Option<VoiceProgress> {
