...
pub(crate) fn absolute_path_checked(path: impl AsRef<Path>) -> io::Result<AbsolutePathBuf> {
...
/// interpreting them using the operating system running this process. Fallback URIs
...
/// requires a valid `file:` URI so remote protocol callers do not rely on the
/// server host's native path syntax. Valid `file:` strings round-trip through
/// their canonical URL form, including encoded non-UTF-8 path bytes, but
/// conversion to a native path remains host-dependent as described by [RFC 8089].
///
...
impl PathUri {
    ...
    pub fn from_path(path: impl AsRef<Path>) -> io::Result<Self> {
        let path = absolute_path_checked(path)
    ...
    pub fn to_abs_path(&self) -> io::Result<AbsolutePathBuf> {
        ...
        if let Some(path_bytes) = decode_bad_path_uri(&self.0) {
            ...
            if let Some(decoded_path) = decoded_path
                && let Ok(path) = absolute_path_checked(decoded_path)
        ...
        absolute_path_checked(path).map_err(|_| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                PathUriParseError::InvalidFileUriPath {
                    path: self.to_string(),
