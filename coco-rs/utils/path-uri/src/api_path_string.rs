...
use crate::PathUri;
use crate::absolute_path_checked;
...
/// boundary while Coco migrates to [`PathUri`].
///
...
/// serde: Coco-internal code cannot construct this type directly from a raw
...
impl TryFrom<LegacyAppPathString> for AbsolutePathBuf {
    ...
    fn try_from(path: LegacyAppPathString) -> Result<Self, Self::Error> {
        absolute_path_checked(path.as_str()).map_err(|_| {
            LegacyAppPathStringError::InvalidNativePath {
                path: path.0,
                convention: None,
            }
