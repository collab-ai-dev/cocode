use super::*;
use coco_utils_absolute_path::AbsolutePathBufGuard;
...
fn file_uri_falls_back_for_windows_prefixes_without_a_uri_representation() {
    for (native_path, expected_uri) in [
        ...
        let path = AbsolutePathBuf::from_absolute_path(native_path)
...
fn file_uri_fallback_round_trips_non_unicode_windows_paths() {
    ...
    let path = AbsolutePathBuf::from_absolute_path(path).expect("absolute Windows path");
...
fn file_uri_falls_back_for_posix_paths_with_null_bytes() {
    ...
    let path = AbsolutePathBuf::from_absolute_path(path).expect("absolute POSIX path");
...
fn ordinary_bad_path_uri_is_not_decoded_as_a_fallback() {
    let path = AbsolutePathBuf::from_absolute_path("/bad/path/L3RtcC9udWxsLQAt_y1ieXRl")
...
fn file_uri_round_trips_windows_unc_paths() {
    let path = AbsolutePathBuf::from_absolute_path(r"\\server\share\src\main.rs")
...
#[test]
fn path_uri_deserialization_requires_file_uri() {
    let path = AbsolutePathBuf::current_dir()
        .expect("current directory")
        .join("workspace/src");
    let json = serde_json::to_string(&path).expect("absolute path should serialize");
    let error = serde_json::from_str::<PathUri>(&json)
        .expect_err("native absolute path JSON should be rejected");
...
fn path_uri_rejects_relative_strings_during_deserialization() {
    ...
    let error = serde_json::from_str::<PathUri>(r#""src/lib.rs""#)
        .expect_err("relative URI string should be rejected");
