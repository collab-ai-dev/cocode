...
fn renders_an_absolute_path_using_the_host_convention() {
    ...
    let path =
        AbsolutePathBuf::from_absolute_path(native_path).expect("native path should be absolute");
...
fn renders_native_non_unicode_windows_fallback_lossily() {
    ...
    let native_path =
        AbsolutePathBuf::from_absolute_path(native_path).expect("absolute native path");
