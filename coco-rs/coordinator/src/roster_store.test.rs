...
fn bootstrap_request(
    ...
    coco_tool_runtime::InitializeSessionTeamRequest {
        ...
        leader_session_id: SessionId::try_new(session_id).unwrap(),
...
async fn initialize_session_team_writes_leader_only_roster() {
    ...
    let typed_session_id = SessionId::try_new(session_id).unwrap();
    let team_name = session_team_name(&typed_session_id);
...
#[test]
fn initialize_session_team_request_rejects_empty_session_id_at_serde_boundary() {
    let err = serde_json::from_value::<coco_tool_runtime::InitializeSessionTeamRequest>(
        serde_json::json!({
            "team_name": "session-",
            "leader_session_id": "",
            "cwd": "/tmp/project"
...
#[test]
fn initialize_session_team_request_rejects_unsafe_session_id_at_serde_boundary() {
    let err = serde_json::from_value::<coco_tool_runtime::InitializeSessionTeamRequest>(
        serde_json::json!({
            "team_name": "session-bad",
            "leader_session_id": "bad/session",
            "cwd": "/tmp/project"
        }),
    )
    .expect_err("unsafe session id rejected");
