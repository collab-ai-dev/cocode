...
fn lock_at(dir: &std::path::Path) -> MaintenanceLock {
    MaintenanceLock::new(dir, LOCK)
}
...
fn first_acquire_returns_acquired_with_zero_prior() {
    ...
    match lock.try_acquire() {
        MaintenanceLockOutcome::Acquired(guard) => {
...
fn second_acquire_from_same_process_is_reclaimable() {
    ...
    let first = match lock.try_acquire() {
        MaintenanceLockOutcome::Acquired(g) => {
            g.commit();
            g
        }
    ...
    assert!(
        matches!(second, MaintenanceLockOutcome::Acquired(_)),
...
fn rollback_with_zero_removes_lock() {
    ...
    if let MaintenanceLockOutcome::Acquired(g) = lock.try_acquire() {
        drop(g);
    }
...
fn lock_guard_rollback_now_restores_prior_mtime() {
    ...
    match lock.try_acquire() {
        MaintenanceLockOutcome::Acquired(g) => g.rollback_now(),
        other => panic!("expected Acquired, got {other:?}"),
    }
    let mtime = lock.last_run_at().unwrap();
...
fn lock_guard_drop_without_commit_rolls_back() {
    ...
    let lock = lock_at(temp.path());
    match lock.try_acquire() {
        MaintenanceLockOutcome::Acquired(g) => g.commit(),
        other => panic!("expected first Acquired, got {other:?}"),
    }
    let prior = lock.last_run_at().unwrap();
    ...
    std::fs::remove_file(temp.path().join(LOCK)).unwrap();
    {
        ...
        let cur = lock.last_run_at().unwrap();
        assert!(cur >= prior);
        drop(g);
    }
    // After rollback-on-drop with prior 0, the file is unlinked.
    assert!(lock.last_run_at().is_none());
}
...
fn fresh_lock_held_by_live_foreign_process_is_held() {
    ...
    let _ = child.wait();
    assert!(
        matches!(outcome, MaintenanceLockOutcome::Held),
