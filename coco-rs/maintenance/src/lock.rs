//! Parameterized PID + mtime CAS lock for maintenance passes.
//!
//! The lock file's **mtime** is the "last successful run" timestamp; its body
...
//! re-create leaves us with [`MaintenanceLockOutcome::Held`]. The stat→unlink reclaim
...
//! ## RAII guard
//!
//! [`MaintenanceGuard`] holds the lock for the duration of a maintenance attempt.
...
//! marked the run committed via [`MaintenanceGuard::commit`].
...
#[derive(Debug)]
pub enum MaintenanceLockOutcome {
    /// Lock acquired. The `MaintenanceGuard` releases (or rolls back) on drop — call
    /// [`MaintenanceGuard::commit`] to make the held mtime stick when the run
    /// succeeded.
    Acquired(MaintenanceGuard),
...
/// A cross-process maintenance lock at a caller-chosen path.
///
/// Construct with [`MaintenanceLock::new`], passing the directory and the lock
...
impl MaintenanceLock {
    ...
    pub fn try_acquire(&self) -> MaintenanceLockOutcome {
        let prior_mtime_ms = read_mtime_ms(&self.lock_path).unwrap_or(0);

        if let Some(parent) = self.lock_path.parent()
            && let Err(e) = std::fs::create_dir_all(parent)
        {
            return MaintenanceLockOutcome::Error(format!("could not create lock dir: {e}"));
        }

        match self.create_new_with_pid() {
            Ok(()) => MaintenanceLockOutcome::Acquired(self.guard(prior_mtime_ms)),
            Err(e) if e.kind() == std::io::ErrorKind::AlreadyExists => {
                if !self.is_reclaimable() {
                    return MaintenanceLockOutcome::Held;
                }
                ...
                match self.create_new_with_pid() {
                    ...
                    Err(e) => MaintenanceLockOutcome::Error(format!("could not write lock: {e}")),
                }
            }
            Err(e) => MaintenanceLockOutcome::Error(format!("could not write lock: {e}")),
        }
    ...
    /// Last successful maintenance-run timestamp (lock file mtime in ms), or
    /// `None` if no lock has ever been written.
    pub fn last_run_at(&self) -> Option<i64> {
        read_mtime_ms(&self.lock_path)
    }

    fn guard(&self, prior_mtime_ms: i64) -> MaintenanceGuard {
        MaintenanceGuard {
...
/// RAII handle for an acquired maintenance lock.
///
