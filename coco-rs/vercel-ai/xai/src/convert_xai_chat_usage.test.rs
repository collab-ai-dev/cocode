use super::*;
use pretty_assertions::assert_eq;

#[test]
fn none_usage_yields_empty() {
    let usage = convert_xai_chat_usage(None);
    assert_eq!(usage.input_tokens.total(), None);
    assert_eq!(usage.output_tokens.total, None);
    assert!(usage.raw.is_none());
}

#[test]
fn reasoning_tokens_are_additive_to_completion() {
    let raw = XaiChatUsage {
        prompt_tokens: Some(100),
        completion_tokens: Some(40),
        total_tokens: Some(140),
        prompt_tokens_details: None,
        completion_tokens_details: Some(XaiCompletionTokensDetails {
            reasoning_tokens: Some(15),
            ..Default::default()
        }),
    };
    let usage = convert_xai_chat_usage(Some(&raw));
    assert_eq!(usage.input_tokens.total(), Some(100));
    // output total = completion + reasoning; text = completion.
    assert_eq!(usage.output_tokens.total, Some(55));
    assert_eq!(usage.output_tokens.text, Some(40));
    assert_eq!(usage.output_tokens.reasoning, Some(15));
    assert!(usage.raw.is_some());
}

#[test]
fn cached_tokens_within_prompt_total() {
    let raw = XaiChatUsage {
        prompt_tokens: Some(100),
        completion_tokens: Some(10),
        total_tokens: Some(110),
        prompt_tokens_details: Some(XaiPromptTokensDetails {
            cached_tokens: Some(30),
            ..Default::default()
        }),
        completion_tokens_details: None,
    };
    let usage = convert_xai_chat_usage(Some(&raw));
    assert_eq!(usage.input_tokens.total(), Some(100));
    assert_eq!(usage.input_tokens.cache_read(), Some(30));
    assert_eq!(usage.input_tokens.no_cache(), Some(70));
}

#[test]
fn cached_tokens_reported_exclusively() {
    // cached_tokens > prompt_tokens → the API reports them exclusively, so the
    // total becomes prompt + cache.
    let raw = XaiChatUsage {
        prompt_tokens: Some(70),
        completion_tokens: Some(10),
        total_tokens: Some(110),
        prompt_tokens_details: Some(XaiPromptTokensDetails {
            cached_tokens: Some(80),
            ..Default::default()
        }),
        completion_tokens_details: None,
    };
    let usage = convert_xai_chat_usage(Some(&raw));
    assert_eq!(usage.input_tokens.total(), Some(150));
    assert_eq!(usage.input_tokens.no_cache(), Some(70));
    assert_eq!(usage.input_tokens.cache_read(), Some(80));
}

#[test]
fn no_reasoning_details_gives_all_text() {
    let raw = XaiChatUsage {
        prompt_tokens: Some(10),
        completion_tokens: Some(20),
        total_tokens: Some(30),
        prompt_tokens_details: None,
        completion_tokens_details: None,
    };
    let usage = convert_xai_chat_usage(Some(&raw));
    assert_eq!(usage.output_tokens.total, Some(20));
    assert_eq!(usage.output_tokens.text, Some(20));
    assert_eq!(usage.output_tokens.reasoning, None);
}
