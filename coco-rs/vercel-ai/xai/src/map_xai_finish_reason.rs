...
/// Map an xAI Chat Completions `finish_reason` to an SDK `FinishReason`.
///
/// Mirrors `map-xai-finish-reason.ts`.
pub fn map_xai_finish_reason(finish_reason: Option<&str>) -> FinishReason {
...
#[cfg(test)]
#[path = "map_xai_finish_reason.test.rs"]
mod tests;
