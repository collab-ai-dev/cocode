...
/// Convert a `LanguageModelV4Prompt` into xAI Chat Completions messages.
///
/// Mirrors `convert-to-xai-chat-messages.ts`. Returns `Ok((messages, warnings))`.
...
pub fn convert_to_xai_chat_messages(
    ...
    for msg in prompt {
        match msg {
            // Grok chat exposes only system/user/assistant/tool roles, so
            // developer messages collapse into a system message.
            ...
            LanguageModelV4Message::Assistant { content, .. } => {
                ...
                for part in content {
                    match part {
                        ...
                        // Other assistant parts have no xAI chat wire
...
/// Convert a single user content part to xAI's wire shape.
        ...
        UserContentPart::File(file) => {
            if !file.media_type.starts_with("image/") {
                return Err(AISdkError::new(format!(
                    "Unsupported functionality: file part media type {}",
                    file.media_type
                )));
            }
            ...
            if let Some(detail) = xai_image_detail(file) {
                image_url_obj["detail"] = Value::String(detail);
            }
...
/// Read the xAI `imageDetail` provider option (`low` | `high` | `auto`) off an
/// image file part's `provider_metadata["xai"]`. Invalid values are dropped —
/// the xAI API silently ignores them, so it is validated client-side.
fn xai_image_detail(file: &FilePart) -> Option<String> {
    let detail = file
        ...
        .get("imageDetail")?
        .as_str()?;
    matches!(detail, "low" | "high" | "auto").then(|| detail.to_string())
}
...
///
/// Mirrors the `switch (output.type)` in `convert-to-xai-chat-messages.ts`.
/// Multimodal `content` outputs degrade non-text parts to a visible marker
/// (matching the openai / openai-compatible / groq siblings) rather than
/// dumping raw base64 into the prompt.
...
#[cfg(test)]
#[path = "convert_to_xai_chat_messages.test.rs"]
mod tests;
