...
fn system_message_collapses_to_string() {
    ...
    let (messages, warnings) = convert_to_groq_chat_messages(&prompt).unwrap();
    assert_eq!(
        messages,
...
fn single_text_user_message_is_plain_string() {
    ...
    let (messages, _) = convert_to_groq_chat_messages(&prompt).unwrap();
...
fn non_image_file_part_errors() {
    ...
    assert!(convert_to_groq_chat_messages(&prompt).is_err());
}

#[test]
fn assistant_message_with_reasoning_and_tool_call() {
