...
Streaming transcription (real-time WebSocket STT): `TranscriptionModelV4::do_stream` is an **optional** defaulted trait method (rejects with an unsupported-functionality error unless overridden, mirroring the spec's `doStream?`). Types: `TranscriptionModelV4StreamOptions` (a live `AudioChunkStream` = `Pin<Box<dyn Stream<Item = Vec<u8>>>>` + `TranscriptionInputAudioFormat`), `TranscriptionModelV4StreamPart` (kebab-case tagged: `stream-start` / `transcript-delta` / `transcript-partial` / `transcript-final` / `response-metadata` / `finish` / `raw` / `error`), `TranscriptionModelV4StreamResult`. Implemented by `vercel-ai-xai` (`/stt` WebSocket).
