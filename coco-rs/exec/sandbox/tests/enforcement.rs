...
// ==========================================================================
// macOS enforcement tests (Seatbelt / sandbox-exec)
// ==========================================================================
//
// Mirrors the Linux module above for the macOS Seatbelt backend. Skips
// gracefully when `/usr/bin/sandbox-exec` is absent. NOTE: this module is
// `cfg(target_os = "macos")`, so it is not compiled on Linux CI — it must be
// exercised on a macOS host (or in macOS CI) to validate.
...
mod macos {
    ...
    #[tokio::test]
    async fn test_dyld_env_hardening() -> anyhow::Result<()> {
        skip_if_unavailable!();
        // macOS `wrap_command` strips the dyld injection vectors before exec.
        let config = SandboxConfig {
            enforcement: EnforcementLevel::ReadOnly,
            allow_network: true,
            ..Default::default()
        ...
        let platform = create_platform();
        let mut cmd = tokio::process::Command::new("/bin/sh");
        cmd.arg("-c").arg("echo \"DI=$DYLD_INSERT_LIBRARIES\"");
        cmd.env("DYLD_INSERT_LIBRARIES", "/evil/lib.dylib");
        cmd.stdout(std::process::Stdio::piped())
            .stderr(std::process::Stdio::piped());
        ...
        assert_eq!(
            stdout, "DI=",
            "DYLD_INSERT_LIBRARIES should be cleared, got: {stdout}"
        );
        Ok(())
    }
}
