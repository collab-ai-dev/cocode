...
const MAX_BUFFERED_HTTP_RESPONSE_BYTES: usize = 64 * 1024 * 1024;
...
async fn read_limited_response_body(
    ...
    while let Some(chunk) = body.next().await {
        ...
        if bytes.len().saturating_add(chunk.len()) > MAX_BUFFERED_HTTP_RESPONSE_BYTES {
            return Err(invalid_params(format!(
                "http/request buffered response body exceeds {MAX_BUFFERED_HTTP_RESPONSE_BYTES} bytes; use streamResponse instead"
            )));
        }
