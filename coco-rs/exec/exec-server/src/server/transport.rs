    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ExecServerListenUrlParseError::NonLoopbackWebSocketListenUrl(listen_url) => write!(
                f,
                "unsafe websocket --listen URL `{listen_url}`; coco exec-server v1 only listens on loopback addresses"
            ),
        }
    }
}
    ...
    W: AsyncWrite + Unpin + Send + 'static,
{
    ...
    tracing::info!("coco-exec-server listening on stdio");
    processor
...
async fn run_websocket_listener(
    ...
    let processor = ConnectionProcessor::new(runtime_paths);
    info!("coco-exec-server listening on ws://{local_addr}");
