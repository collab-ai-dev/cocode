...
#[tokio::test]
async fn process_start_rejects_sandbox_context() {
    let handler = initialized_handler().await;
    let mut params = exec_params("proc-sandbox");
    params.sandbox = Some(FileSystemSandboxContext {
        value: serde_json::json!({"mode": "sandbox"}),
    });

    let error = handler
        .exec(params)
        .await
        .expect_err("sandboxed process execution should be rejected");
    assert_eq!(error.code, -32602);
    assert_eq!(
        error.message,
        "sandboxed process execution is unsupported by coco exec-server v1"
    );
    handler.shutdown().await;
}

#[tokio::test]
async fn process_start_rejects_managed_network_without_sandbox() {
    let handler = initialized_handler().await;
    let mut params = exec_params("proc-managed-network");
    params.enforce_managed_network = true;

    let error = handler
        .exec(params)
        .await
        .expect_err("managed network should require unsupported sandbox execution");
    assert_eq!(error.code, -32602);
    assert_eq!(
        error.message,
