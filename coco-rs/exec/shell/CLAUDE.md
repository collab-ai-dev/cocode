...
Shell execution with security analysis, destructive-command warnings, sandbox decisions, read-only / mode validation, heredoc + sed-in-place handling, CWD tracking, snapshot capture, hook-emitted session env, and `/env` vars. Builds on `coco-shell-parser` (tree-sitter + tokenizer) for parsing and the risk analyzers; the `SecurityCheckId` constants and `check_security` entry point are defined in **this** crate (`src/safety.rs`, `src/security.rs`), which layers read-only validation, destructive warnings, mode validation, and provider-based command assembly on top.
...
- **Parser + risk analyzers** live in `coco-shell-parser` (infrastructure). This crate layers read-only validation, destructive warnings, mode validation, sed edit detection, accept-edits auto-allow, and sandbox decision on top.
...
- **Security**: `SecurityCheck`, `SecurityCheckId`, `SecuritySeverity`, `SafetyResult`, `check_security`
...
**Sandbox fail-closed invariant:** if a command should be wrapped and wrapping fails, foreground shell and background task runners must refuse to spawn the original command unsandboxed.
