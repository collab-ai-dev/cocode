...
#[tokio::test]
async fn unified_diff_for_cr_only_file_has_valid_lf_records() {
    let dir = tempdir().unwrap();
    let path = dir.path().join("classic-mac.txt");
    fs::write(&path, "alpha\rbeta\rgamma\r").unwrap();
    let patch = parse_patch(&wrap_patch(&format!(
        "*** Update File: {}\n@@\n alpha\n-beta\n+BETA\n gamma",
        path.display()
    )))
    .unwrap();
    let chunks = match patch.hunks.as_slice() {
        [Hunk::UpdateFile { chunks, .. }] => chunks,
        _ => panic!("expected one update hunk"),
    };
    let path_uri = PathUri::from_path(&path).expect("absolute test path");

    let update = unified_diff_from_chunks_with_mode(
        &path_uri,
        chunks,
    ...
    assert!(!update.unified_diff.contains('\r'));
    assert!(update.unified_diff.contains("-beta\n+BETA\n"));
    assert_eq!(update.original_content, "alpha\rbeta\rgamma\r");
    assert_eq!(update.content, "alpha\rBETA\rgamma\r");
}
