...
async fn unified_diff_from_chunks_with_context_and_mode(
    ...
    // `similar::TextDiff::from_lines` recognizes LF boundaries. Normalize
    // only the presentation inputs so CR-only and CRLF files produce a valid
    // unified diff while the returned old/new contents retain exact endings.
    let diff_original = normalize_line_endings_for_diff(&original_contents);
    let diff_new = normalize_line_endings_for_diff(&new_contents);
    let text_diff = TextDiff::from_lines(&diff_original, &diff_new);
...
fn normalize_line_endings_for_diff(contents: &str) -> String {
