...
| Multi-session AppServer architecture: ProcessRuntime/SessionRuntime/SessionHandle split, `LiveSessionRegistry` slots + replace 2PC, `SessionEnvelope` + per-session `session_seq`, surfaces/adapters, SDK contract, hub v2 reconciliation, cut-over plan | `multi-session-app-server-plan.md` | crate plans own stable type definitions once crates land; `event-hub/spec.md` §4 identity model is superseded by this doc's §13 and must be revised with the cut-over |
