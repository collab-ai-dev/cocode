...
| **MCP tool exposure** (`load`/`defer`/`use_tool`), global default + per-server overrides, `Loaded`/`Deferred`/`UseTool` placement, `alwaysLoad`, canonical `WireToolName`, carrier unwrap, bounded search projection | `mcp-tool-exposure-design.md` | `tool-search-design.md` owns the query DSL/scoring; `crate-coco-mcp.md` owns protocol/config internals |
...
| RTK integration: `Feature::Rtk`, `RtkConfig`/`RtkEngine`, two-tier design (phase-1 `rtk rewrite` subprocess, phase-2 embedded post-exec filter core), shared `.rtk/filters.toml` + trust store, grep/glob native absorptions | `rtk-integration-design.md` | crate docs (`crate-coco-tools.md`, `crate-coco-shell.md`, `crate-coco-config.md`) reference, not redefine |
| Multi-session AppServer architecture: verified current state, explicit-target protocol, runtime ownership, lifecycle, surfaces, and remediation gates | `multi-session-app-server/README.md` | `current-architecture.md` is descriptive; `target-architecture.md` and `protocol-scope.md` are normative; crate docs own stable crate-local details and `event-hub/spec.md` owns Hub wire details |
| Persistent goal implementation audit, three-project state/resume/Plan/completion/TUI comparison, hard completion coordination and gate, context and plan handoff, liveness defects, and direct final-architecture cutover | `goal-architecture-redesign.md` | crate docs own final field-level type definitions; this doc owns cross-cutting goal lifecycle, authority, scheduling, context, plan integration, completion coordination, TUI behavior, and migration decisions |
| Goal-runtime live-integration plan: built-vs-remaining status, trait→concrete-impl seam inventory, per-phase wiring order (store/handle ownership, tools, protocol, TUI, Stop-Hook removal) with verified engine seams | `goal-runtime-integration-plan.md` | `goal-architecture-redesign.md` remains normative for the *what*; this doc owns the *wiring order*; `core/goals` + `core/goal-runtime` crate docs own the built type definitions |
| Sidechat (`/btw`) architecture: ephemeral child-session model, read-only tool enforcement, hook policy, Hub exclusion, session-scoped TUI projection, local authority, and lifecycle invariants | `sidechat-architecture.md` | `event-hub/spec.md` owns Hub wire encoding; `multi-session-app-server/` owns generic AppServer lifecycle; this document owns sidechat's cross-cutting participation and verification contracts |
...
| `mcp-tool-exposure-design.md` | Implemented server-level MCP exposure (`load`/`defer`/`use_tool`), global default + per-server overrides, `alwaysLoad` precedence, canonical `WireToolName`, typed request snapshots, the `use_tool` carrier with dual provider/semantic identity, bounded search projection, and never-widen inheritance. |
...
| `multi-session-app-server/` | Multi-session AppServer documentation set: verified review, current architecture, normative breaking target and protocol scope, executable remediation gates, and concise migration history. |
| `sidechat-architecture.md` | Implemented sidechat (`/btw`) architecture and adversarial review: real ephemeral child session, reservation-first parent-owned lifecycle, structurally enforced read-only tools, exhaustive hook allowlist, bounded context, local-only egress, cache-detector namespaces, session-scoped TUI projection, remote rejection, and verification map. |
| `goal-architecture-redesign.md` | Goal audit against Hermes/Codex, including state/resume, Plan mode, mandatory completion coordination, exclusive evidence gate, TUI control plane, worker context, fork vs summary behavior, plan-file and system-reminder handoff, proven liveness defects, and a direct cutover to the first-class session goal architecture with no Stop Hook bridge. |
| `goal-runtime-integration-plan.md` | Executable wiring plan for the goal runtime: what ships in `coco-goals` / `coco-goal-runtime` / `coco-session` today (tested), the trait→concrete-impl seam inventory, and the per-phase live-integration order (SessionRuntime ownership + lease, goal tools, protocol/AppServer, TUI, Stop-Hook removal) with verified engine seam references. |
