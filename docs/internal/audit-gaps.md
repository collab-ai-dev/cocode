...
| **Deny-read glob enforcement** | sandbox | `glob_expansion` owns one portable validator. Linux performs literal-prefix walks with depth 64, entry/match caps, canonical-target masking, and fail-closed errors before bwrap; macOS emits anchored Seatbelt regexes that also cover post-launch files and path aliases. Adapter classification remains in `denied_read_globs`. | **RESOLVED** |
